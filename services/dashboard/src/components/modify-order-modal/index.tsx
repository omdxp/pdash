...
interface ModifyOrderModalProps {
  order: Order;
  submit: (draftOrder: Order) => void;
  cancel: () => void;
  deleteOrder: () => void;
}

const ModifyOrderModal: Component<ModifyOrderModalProps> = ({
  order,
  submit,
  cancel,
  deleteOrder,
  ...
  const [draftOrder, setDraftOrder] = createSignal(order);
  onMount(() => {
    setDraftOrder(order);
  });
  return (
